...
def get_top_x_movies_by_rank(x: int, results: list):
    ...
    top_movies = []
    for x in actors:
        top_actors.append(x[0])
    for x in movies:
        top_movies.append(x[0])
...
def get_summary_with_snippet(movie_info, query):
    ...
    if snippet == '':
        for word in query.split(" "):
            summary = summary.lower().replace(
                word,
                f"<b><font size='4' color={random.choice(list(color)).value}>{word}</font></b>",
            )
    else:
        summary = snippet
        if "***" in snippet:
...
def search_handling(
    ...
    if search_button:
        all_docs = set()
        for doc in utils.movies_dataset:
            if doc['first_page_summary']:
                for term in doc['first_page_summary'].split(" "):
                    all_docs.add(term)
            if doc['genres']:
            ...
            if doc['summaries']:
                for s in doc['summaries']:
                    for term in s.split(" "):
                        all_docs.add(term)
        corrected_query = utils.correct_text(search_term, all_docs)
