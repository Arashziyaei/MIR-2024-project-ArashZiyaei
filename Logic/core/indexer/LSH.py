...
class MinHashLSH:
    ...
    def min_hash_signature(self):
        ...
        # TODO
        def hash_function(x, coefficient, constant):
            hash_value = 0
            for char in x:
                hash_value = (hash_value * coefficient + ord(char) + 12345) % 1e9+7
            hashed_value = (hash_value + constant) % 1e9+7
            return hashed_value

        num_docs = len(self.documents)
        signature_matrix = np.full((self.num_hashes, num_docs), np.inf)

        coefficients = [random.randint(0, (1<<32)) for _ in range(self.num_hashes)]
        constants = [random.randint(0, (1<<32)) for _ in range(self.num_hashes)]

        for doc_index, document in enumerate(self.documents):
            shingles = self.shingle_document(document)
            for shingle in shingles:
                for hash_index in range(self.num_hashes):
                    hash_value = hash_function(' '.join(shingle), coefficients[hash_index], constants[hash_index])
                    signature_matrix[hash_index][doc_index] = min(signature_matrix[hash_index][doc_index], hash_value)
        ...
        # hash_functions = [self.generate_hash_function() for _ in range(self.num_hashes)]
        #
        # # Iterate over each row (shingle) in the characteristic matrix
        # for row_index in range(num_shingles):
        #     # Check if this shingle is present in each document
        #     for doc_index in range(num_docs):
        #         if characteristic_matrix[row_index][doc_index] == 1:
        #             # Update the signature matrix with minimum hash values
        #             for hash_index, hash_function in enumerate(hash_functions):
        #                 hash_value = hash_function(row_index)
...
if __name__ == '__main__':
    ...
    crawled_files = crawled_files[:500]
